...
#include "vkh/systems/hud/elements/element.hpp"
...
void animateBubbly(vkh::EngineContext &context, vkh::hud::Element &element) {
  for (const auto &child : element.children) {
    if (auto btn = std::dynamic_pointer_cast<UIBtn>(child)) {
...
UIBtn::UIBtn(vkh::hud::View &view, Element *parent, glm::vec2 position,
  ...
  outer = addChild<vkh::hud::Polygon>(outerVertices, 0);
  inner = addChild<vkh::hud::Polygon>(innerVertices, 0);
  this->label = addChild<vkh::hud::Text>(glm::vec2{}, label);
}

void UIBtn::animate(float time) {
  ...
  static thread_local std::vector<vkh::hud::Polygon::Vertex> ov = outerVertices;
  static thread_local std::vector<vkh::hud::Polygon::Vertex> iv = innerVertices;
