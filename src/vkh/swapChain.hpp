...
public:
  ...
  // Swapchain images are now created directly as UNORM (not SRGB), so the

  // same view supports both color-attachment and storage-image usage —

  // no separate aliased view needed. Kept as a distinct getter so call

  // sites stay explicit about which usage they mean.
