...
namespace vkh {
	...
	{
		if (key == GLFW_KEY_A) keys.moveLeft = static_cast<bool>(action);
		if (key == GLFW_KEY_D) keys.moveRight = static_cast<bool>(action);
		if (key == GLFW_KEY_W) keys.moveForward = static_cast<bool>(action);
		if (key == GLFW_KEY_S) keys.moveBackward = static_cast<bool>(action);
		if (key == GLFW_KEY_SPACE) keys.moveUp = static_cast<bool>(action);
		if (key == GLFW_KEY_LEFT_CONTROL) keys.moveDown = static_cast<bool>(action);
		...
		if (key == GLFW_KEY_F3) keys.showDebugInfo = static_cast<bool>(action);
	}
	glm::vec2 mousePos;
	...
	void Controller::moveInPlaneXZ(
		EngineContext& context, float dt, Entity viewerEntity) {
		auto& cameraRotation = context.ecs.getComponent<Transform>(viewerEntity).rotation;
		...
		if (keys.moveForward) moveDir += forwardDir;
		if (keys.moveBackward) moveDir -= forwardDir;
		if (keys.moveRight) moveDir += rightDir;
		if (keys.moveLeft) moveDir -= rightDir;
		if (keys.moveUp) moveDir += upDir;
		if (keys.moveDown) moveDir -= upDir;

		if (glm::dot(moveDir, moveDir) > std::numeric_limits<float>::epsilon()) {
			context.ecs.getComponent<Transform>(viewerEntity).translation += moveSpeed * dt * glm::normalize(moveDir);
		}
	}
}  // namespace lve