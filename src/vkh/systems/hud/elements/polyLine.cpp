...
void PolyLine::addPoint(glm::vec2 point) {
  ...
  if (parent) {
    setAbsolutePosition(parent->getAbsolutePosition() + minP * parent->getAbsoluteSize());
...
void PolyLine::addToDrawInfo(DrawInfo &drawInfo, float depth) {
  ...
  for (size_t i = 0; i < points.size(); ++i) {
    glm::vec2 absPos = parent->getAbsolutePosition() + points[i] * parent->getAbsoluteSize();
...
bool PolyLine::isPositionInside(const glm::vec2 &pos) {
  ...
  for (size_t i = 0; i < points.size() - 1; ++i) {
    glm::vec2 p1 = parent->getAbsolutePosition() + points[i] * parent->getAbsoluteSize();
    glm::vec2 p2 = parent->getAbsolutePosition() + points[i + 1] * parent->getAbsoluteSize();
