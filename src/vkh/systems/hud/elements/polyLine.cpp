...
namespace vkh::hud {
PolyLine::PolyLine(View &view, Element *parent, glm::vec2 position,
                   glm::vec3 color)
...
void PolyLine::addPoint(glm::vec2 point) {
  ...
  // Update bounding box (Element::position and Element::size)
  ...
  // Element position/size are in absolute screen space
  // points are relative to canvas
  if (parent) {
    position = parent->position + minP * parent->size;
    size = (maxP - minP) * parent->size;
  }
}

void PolyLine::addToDrawInfo(DrawInfo &drawInfo, float depth) {
  ...
  for (size_t i = 0; i < points.size(); ++i) {
    glm::vec2 absPos = parent->position + points[i] * parent->size;
...
bool PolyLine::isPositionInside(const glm::vec2 &pos) {
  ...
  const glm::vec2 minB = glm::min(position, position + size);
  const glm::vec2 maxB = glm::max(position, position + size);
  ...
  for (size_t i = 0; i < points.size() - 1; ++i) {
    glm::vec2 p1 = parent->position + points[i] * parent->size;
    glm::vec2 p2 = parent->position + points[i + 1] * parent->size;
