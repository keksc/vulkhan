...
namespace vkh {
    ...
    LveSwapChain::LveSwapChain(
        EngineContext& context, VkExtent2D windowExtent, std::shared_ptr<LveSwapChain> previous)
    ...
    LveSwapChain::~LveSwapChain() {
        ...
        for (int i = 0; i < depthImages.size(); i++) {
            vkDestroyImageView(context.vulkan.device, depthImageViews[i], nullptr);
            vkDestroyImage(context.vulkan.device, depthImages[i], nullptr);
            vkFreeMemory(context.vulkan.device, depthImageMemorys[i], nullptr);
        }
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            vkDestroySemaphore(context.vulkan.device, renderFinishedSemaphores[i], nullptr);
            vkDestroySemaphore(context.vulkan.device, imageAvailableSemaphores[i], nullptr);
    ...
    VkResult LveSwapChain::submitCommandBuffers(const VkCommandBuffer* buffers, uint32_t* imageIndex) {
        if (imagesInFlight[*imageIndex] != VK_NULL_HANDLE) {
            vkWaitForFences(context.vulkan.device, 1, &imagesInFlight[*imageIndex], VK_TRUE, UINT64_MAX);
        }
        ...
        if (vkQueueSubmit(context.vulkan.graphicsQueue, 1, &submitInfo, inFlightFences[currentFrame]) !=
            VK_SUCCESS) {
    ...
    void LveSwapChain::createSwapChain() {
        ...
        if (vkCreateSwapchainKHR(context.vulkan.device, &createInfo, nullptr, &swapChain) != VK_SUCCESS) {
        ...
        vkGetSwapchainImagesKHR(context.vulkan.device, swapChain, &imageCount, nullptr);
        swapChainImages.resize(imageCount);
        vkGetSwapchainImagesKHR(context.vulkan.device, swapChain, &imageCount, swapChainImages.data());
    ...
    void LveSwapChain::createImageViews() {
        ...
        for (size_t i = 0; i < swapChainImages.size(); i++) {
            ...
            if (vkCreateImageView(context.vulkan.device, &viewInfo, nullptr, &swapChainImageViews[i]) !=
                VK_SUCCESS) {
    ...
    void LveSwapChain::createRenderPass() {
        ...
        if (vkCreateRenderPass(context.vulkan.device, &renderPassInfo, nullptr, &renderPass) != VK_SUCCESS) {
    ...
    void LveSwapChain::createDepthResources() {
        ...
        for (int i = 0; i < depthImages.size(); i++) {
            ...
            if (vkCreateImageView(context.vulkan.device, &viewInfo, nullptr, &depthImageViews[i]) != VK_SUCCESS) {
                throw std::runtime_error("failed to create texture image view!");
            }
        }
    }
    ...
    void LveSwapChain::createSyncObjects() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            if (vkCreateSemaphore(context.vulkan.device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) !=
                VK_SUCCESS ||
                vkCreateSemaphore(context.vulkan.device, &semaphoreInfo, nullptr, &renderFinishedSemaphores[i]) !=
                VK_SUCCESS ||
                vkCreateFence(context.vulkan.device, &fenceInfo, nullptr, &inFlightFences[i]) != VK_SUCCESS) {
